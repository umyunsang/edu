...
관계형 데이터모델을 다루는 데이터베이스 시스템을 관계형 데이터베이스 관리시스템(Relational DataBase Management
...
SQL이 아니라는 의미에서 NoSQL이라고 하기도 하고 “Not Only SQL” 이라는 의미에서 전통적인 관계형 데이터베이스의 한

계를 극복하기 위한 다양한 저장 방식을 지원함
...
CAP 이론은 Consistency(일관성), Availability(가용성), Partition Tolerance(분할허용)의 세가지 속성을 분산 시스템에서 동시
...
전통적으로 단일 서버환경(소규모)을 기반으로 설계되었기 때문에 분산 처리보다는 트랜잭션의 일관성 과 데이터 무결성을
...
UPDATE users SET email='uys1705817'
WHERE name='umyunsang';

데이터 삭제

DELETE
FROM users WHERE name='umyunsang';
...
query =
"INSERT INTO courses (name, instructor, location, time) VALUES (%s, %s, %s, %s)"

values =
(data['name'], data['instructor'], data['location'], data['time'])
...
query =
"UPDATE courses SET name=%s, instructor=%s, location=%s, time=%s WHERE id=%s"

values =
(data['name'], data['instructor'], data['location'], data['time'], course_id)
...
query =
"UPDATE students SET name=%s, student_id=%s WHERE id=%s"
