...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow696901" style="font-size:14px;font-weight:600">데이터베이스 관리 시스템 단계</label>
  <div id="noteFlow696901-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">데이터베이스 관리 시스템 핵심 개념</div>
  <input id="noteFlow696901" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow696901 = document.getElementById('noteFlow696901');
    var noteFlow696901Out = document.getElementById('noteFlow696901-out');
    var noteFlow696901Steps = ["데이터베이스 관리 시스템 핵심 개념","데이터베이스 관리 시스템 처리 절차","데이터베이스 관리 시스템 검증·응용"];
    noteFlow696901.addEventListener('input', function () { noteFlow696901Out.textContent = noteFlow696901Steps[Number(noteFlow696901.value) - 1]; });
  </script>
</div>
```
