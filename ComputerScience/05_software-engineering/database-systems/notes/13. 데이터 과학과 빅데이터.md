...
flowchart TD
    ROOT["빅데이터의 5V 핵심 특성"]
    ROOT --> V1["1. 규모 (Volume)<br/>페타바이트(PB)·엑사바이트(EB)급 대용량"]
    ROOT --> V2["2. 속도 (Velocity)<br/>실시간 스트리밍 생성·처리 (IoT, 실시간 로그)"]
    ROOT --> V3["3. 다양성 (Variety)<br/>정형(RDB), 반정형(JSON/XML), 비정형(영상/텍스트)"]
    ROOT --> V4["4. 정확성 (Veracity)<br/>노이즈 필터링 및 데이터 신뢰성 확보"]
    ROOT --> V5["5. 가치 (Value)<br/>비즈니스 의사결정 기여 및 경제적 통찰 창출"]
```
