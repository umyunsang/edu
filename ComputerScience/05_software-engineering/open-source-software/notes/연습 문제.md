...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz ) AMT AAAM TH SAAS HM StHS aye Ip.png -->
```html
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 2 SIMI transition® ASH AA Batt 4+ STs.png -->
- 없다면 순식간에 애니메이션 효과없이 변신한다 -> 360도 회전효과가 보이지 않는다

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 3 AMIS cra tt SS ABH HHA YD vera.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 4 AMA AAHFASHYE A+ dS rotated + & Yo.png -->
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 5 SIMI HAS YS AS dd ys SAEs.png -->
(1) 웹 화면에 출력
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 6 SIMIAN ora img 2) onclick S AFM AH.png -->
```html
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 7 AAI d + 360; HA d - 360; € tho WA.png -->
- 반시계 방향으로 회전한다

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 8 AMZN ore AUS AMD AtHHAAA S eed.png -->
```html
<img id="rotateImage" src="/1.이벤트 이해하기/static/photo2.jpg" onmouseover="small()" onmouseout="back()">
```

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 9 AMZ SAAN AW244S htmis ES BSS.png -->
```html
<script>
i.onmousedown = large;
i.onmouseup = back;
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 10 AMS style lineartt Yl WADs2 Arle.png -->
- linear가 있으면 등속
- linear가 없으면 천천히 시작해서 빠르게 움지이다가 다시 천천히 움직인다

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 1 AMSA OFA EUS AtAltro ArepA ays Efodl.png -->
```html
<div id="sky" onmousemove = "updateImage(event)">
```

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 12 AAS ora tr BSS ABR YHA YD aera.png -->
```js
i.style.transform = "translate(" + X + "px, " + Y + "px)";
```

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 13 AMSUM OFFA HATE AAS AspAL Ay (ApAAr.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 414 AMES SAAN AWSAS html gs Ee + BSS.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 15 Ft EMA TE AMSASY htmis BS A Yer.png -->
```css
        #followMouse {
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 16 AMSA OTA eventS +E 1522 tte.png -->
- 자바 스크립트 내부에서는 다른 이름으로 바꾸어도 가능하지만
- 자바스크립트 밖에 있는 html 태그에서 이벤트 코딩할 때는 괄호 안에 e를 적지말고 event라고 정확히 적어야 한다

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 17 cr ANN HA QE ANS SD AgSs 24s Gs ys.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 18 AMA AAS SYA A.png -->
...
1) 부모 자식 모두 버블링과 캡쳐링 둘 다 구현 후 실행시켜보세요. 팝업창 순서는?
...
2) function 이라는 키워드를 사용하지 않고 함수를 표현한다면?
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 19 ANNA GE Sita cH tt Geist Att Cho AS.png -->
```js
c.addEventListener("click", (e) => {alert("자식 (캡쳐링)"); e.stopPropagation()}, true);
```

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 20 MISE event WAI OLE clientYS ASAD, A.png -->
- clientY : 마우스의 Y 좌표
- deltaY : 마우스 휠의 Y 좌표

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 21 AAS addEventListener (ha AFSAAPY AH.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 22 photoé.pngTt tf 4h LE gq22.htmigd Y.png -->
```html
<!DOCTYPE html>
<html>
<head>
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 23 AME focus HE 41 click “ HES FH 4H A.png -->
- 예를 들어 tab 키 누르면 입력 가능한 곳에 focus 가 움직입니다
 	- 마우스 클릭 없이 focus 가능
- click 으로 바꾸면 텝으로 입력가능한 곳이 선택되지 않는다

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 24 AIMI74IM craat Brel CSSB YE MUS YD.png -->
- link태그 이용해서 따로 파일 분리하는 이유
 	- 여러 html에서 css 파일을 공유할 때
 	- css가 너무 길어서 편의상 파일 분리

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 25 SIMI74I1M label for messageBox °J.png -->
- 라벨을 하면 서식넣기도 좋고 라벨을 클릭할 때 input에 커서가 이동함

---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 26 4IMI7914 e.preventDefault(); Th WE.png -->
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 27 07 HUB ZHAN div YAS BH HAA innerHTM.png -->
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 28 AN74 BLS SIAL.png -->
```html
<!DOCTYPE html>
<html>
<head>
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 29 SIMI7S script HBS crest Pel SGHWSU.png -->
```js
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 30 SIMI8414 select2 4 option¢l a4.png -->
```html
        <select id="selectBox">
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 3) AMGAA AIS A4tsa AS oe Atwe 4LeSs DS.png -->
```html
<!DOCTYPE html>
<html>
<head>
...
---
<!-- 원본 이미지 없음: open-source-software__연습 문제__Quiz 32 SIMONI Heret tb E(ArrowUp, ArrowDow.png -->
```html
<!DOCTYPE html>
<html>
<head>
