...
```html preview
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="aiFlow19" style="font-size:14px;font-weight:600">CNN 설계 모듈 실습 - Conv2d 매개변수 단계</label>
  <div id="aiFlow19-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">in_channels</div>
  <input id="aiFlow19" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF의 순서대로 핵심 단계를 확인합니다.</p>
  <script>
    var aiFlow19 = document.getElementById('aiFlow19');
    var aiFlow19Out = document.getElementById('aiFlow19-out');
    var aiFlow19Steps = ["in_channels","kernel·stride","out_channels"];
    aiFlow19.addEventListener('input', function () {
      aiFlow19Out.textContent = aiFlow19Steps[Number(aiFlow19.value) - 1];
    });
  </script>
</div>
```
