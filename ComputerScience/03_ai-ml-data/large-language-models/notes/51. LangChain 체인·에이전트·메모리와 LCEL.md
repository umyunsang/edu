...
이 구조는 단계별 책임이 분명하지만 앞 단계의 오류가 뒤 단계로 전파될 수 있다.
...
<Tabs>
  <Tab label="Chain">기능과 실행 순서가 미리 정해져 있다. 반복 가능한 변환, 요약, 검색 질의응답에 적합하다.</Tab>
...
> [!CAUTION]
> 에이전트가 터미널·웹 요청·코드 실행 도구를 사용할 때는 입력 검증, 권한 제한, 종료 조건을 별도로 설계해야 한다.
...
동시 실행 수와 출력 순서, 오류 전파 방식을 호출 방식에 맞춰 관리한다.
...
> [!WARNING]
> 원문에는 환경·영향 관련 오탈자, Memory 클래스 이름의 반복 철자 오류, placeholder 제목과 실제 클래스 이름의 불일치가 있다. 일부 Agent·Memory API는 이전 인터페이스를 사용하므로 코드의 개념적 흐름과 현재 실행 가능성을 구분한다.
