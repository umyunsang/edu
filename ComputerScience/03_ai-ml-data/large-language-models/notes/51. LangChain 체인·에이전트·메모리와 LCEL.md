...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow1166012" style="font-size:14px;font-weight:600">LangChain 체인·에이전트·메모리와 LCEL 단계</label>
  <div id="noteFlow1166012-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">LangChain 체인·에이전트·메모리와 LCEL 핵심 개념</div>
  <input id="noteFlow1166012" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow1166012 = document.getElementById('noteFlow1166012');
    var noteFlow1166012Out = document.getElementById('noteFlow1166012-out');
    var noteFlow1166012Steps = ["LangChain 체인·에이전트·메모리와 LCEL 핵심 개념","LangChain 체인·에이전트·메모리와 LCEL 처리 절차","LangChain 체인·에이전트·메모리와 LCEL 검증·응용"];
    noteFlow1166012.addEventListener('input', function () { noteFlow1166012Out.textContent = noteFlow1166012Steps[Number(noteFlow1166012.value) - 1]; });
  </script>
</div>
```
