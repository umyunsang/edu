...
- **로딩**: 텍스트, PDF, 웹, 데이터베이스, API 등에서 자료를 가져오며, LlamaHub의 커넥터도 활용할 수 있다.
- **인덱싱**: 작은 단위로 분할하고 의미를 벡터로 표현한다.
- **저장**: 임베딩과 출처·시간 같은 메타데이터를 재사용 가능한 저장소에 둔다.
- **질의와 평가**: 서브쿼리, 다단계 쿼리, 하이브리드 전략 등으로 관련 문서를 찾고 정확성·충실도·속도를 점검한다.
...
별도 최적화 지침은 약 2,000~3,000자와 약 500자 중첩을 제안한다. 이는 시각화의 실습 설정과 구분되는 별도 권장 예다.
...
OpenAI, Hugging Face, spaCy 계열 임베딩이 예시로 제시된다. 원문은 OpenAI 예시로 text-embedding-ada-002를 들고 1,536차원을 대표 설정으로 설명하지만, 이는 해당 모델에 묶인 속성으로 읽어야 한다.
...
- **미세 조정**은 특정 도메인 지식을 학습·암기한 상태로 만든다.
- **RAG**는 질문 시점에 외부 지식을 찾아 모델이 참고하게 한다.
- RAG는 별도의 재학습 없이 자료를 갱신하고 근거를 제시하기 쉽다.

> [!WARNING]
> 원문 코드에는 URL 줄바꿈과 메타데이터 속성 철자 오류가 있다. 유사도 검색 예시의 ‘격하 과정’도 문맥이 불명확한 오타 가능 문구다. 또한 특정 임베딩 모델명과 차원 설명은 작성 시점에 묶인 정보이므로 현재의 일반 표준처럼 확대 해석하지 않는다.
...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow578531" style="font-size:14px;font-weight:600">RAG의 원리와 인덱싱 단계</label>
  <div id="noteFlow578531-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">RAG의 원리와 인덱싱 핵심 개념</div>
  <input id="noteFlow578531" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow578531 = document.getElementById('noteFlow578531');
    var noteFlow578531Out = document.getElementById('noteFlow578531-out');
    var noteFlow578531Steps = ["RAG의 원리와 인덱싱 핵심 개념","RAG의 원리와 인덱싱 처리 절차","RAG의 원리와 인덱싱 검증·응용"];
    noteFlow578531.addEventListener('input', function () { noteFlow578531Out.textContent = noteFlow578531Steps[Number(noteFlow578531.value) - 1]; });
  </script>
</div>
```
