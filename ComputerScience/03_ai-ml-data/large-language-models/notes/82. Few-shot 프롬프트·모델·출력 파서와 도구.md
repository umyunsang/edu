...
> [!NOTE]
> 이 구간은 **적절한 예시를 고르고, 모델을 호출하고, 결과를 지정한 형식으로 바꾸는 흐름**을 다룬다. 도구와 체인은 구조화된 출력을 다음 행동으로 연결한다.
...
<Tabs>
  <Tab label="고정 Few-shot">미리 정한 예시를 프롬프트에 넣는 구성이다.</Tab>
  <Tab label="동적 Few-shot">ExampleSelector가 새 질문과 의미적으로 가까운 예시를 선택하는 구성이다.</Tab>
</Tabs>
...
원문 예시는 temperature 0.7, frequency penalty 0.5, presence penalty 0.5와 줄바꿈 정지 조건을 사용한다. 값의 허용 범위나 최적값은 제시하지 않는다.
...
- Few-shot은 작업 예시를 제공하며, selector는 새 입력과 관련된 예시를 선택한다.
...
- Tool은 Agent의 행동 단계에서 특정 기능을 수행하고 그 결과를 관찰로 제공한다.
...
```html preview
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow5531315" style="font-size:14px;font-weight:600">Few-shot 프롬프트·모델·출력 파서와 도구 단계</label>
  <div id="noteFlow5531315-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">Few-shot 프롬프트·모델·출력 파서와 도구 핵심 개념</div>
  <input id="noteFlow5531315" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow5531315 = document.getElementById('noteFlow5531315');
    var noteFlow5531315Out = document.getElementById('noteFlow5531315-out');
    var noteFlow5531315Steps = ["Few-shot 프롬프트·모델·출력 파서와 도구 핵심 개념","Few-shot 프롬프트·모델·출력 파서와 도구 처리 절차","Few-shot 프롬프트·모델·출력 파서와 도구 검증·응용"];
    noteFlow5531315.addEventListener('input', function () { noteFlow5531315Out.textContent = noteFlow5531315Steps[Number(noteFlow5531315.value) - 1]; });
  </script>
</div>
```
