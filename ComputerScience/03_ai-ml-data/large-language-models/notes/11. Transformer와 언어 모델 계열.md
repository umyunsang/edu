...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow1524" style="font-size:14px;font-weight:600">Transformer와 언어 모델 계열 단계</label>
  <div id="noteFlow1524-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">Transformer와 언어 모델 계열 핵심 개념</div>
  <input id="noteFlow1524" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow1524 = document.getElementById('noteFlow1524');
    var noteFlow1524Out = document.getElementById('noteFlow1524-out');
    var noteFlow1524Steps = ["Transformer와 언어 모델 계열 핵심 개념","Transformer와 언어 모델 계열 처리 절차","Transformer와 언어 모델 계열 검증·응용"];
    noteFlow1524.addEventListener('input', function () { noteFlow1524Out.textContent = noteFlow1524Steps[Number(noteFlow1524.value) - 1]; });
  </script>
</div>
```
