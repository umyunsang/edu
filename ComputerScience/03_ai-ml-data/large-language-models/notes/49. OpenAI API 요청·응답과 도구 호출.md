...
```html preview
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow1226411" style="font-size:14px;font-weight:600">OpenAI API 요청·응답과 도구 호출 단계</label>
  <div id="noteFlow1226411-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">OpenAI API 요청·응답과 도구 호출 핵심 개념</div>
  <input id="noteFlow1226411" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow1226411 = document.getElementById('noteFlow1226411');
    var noteFlow1226411Out = document.getElementById('noteFlow1226411-out');
    var noteFlow1226411Steps = ["OpenAI API 요청·응답과 도구 호출 핵심 개념","OpenAI API 요청·응답과 도구 호출 처리 절차","OpenAI API 요청·응답과 도구 호출 검증·응용"];
    noteFlow1226411.addEventListener('input', function () { noteFlow1226411Out.textContent = noteFlow1226411Steps[Number(noteFlow1226411.value) - 1]; });
  </script>
</div>
```
