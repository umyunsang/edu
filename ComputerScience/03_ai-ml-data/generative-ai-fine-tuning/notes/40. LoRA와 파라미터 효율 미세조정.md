...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai40" style="font-size:14px;font-weight:600">40. LoRA와 파라미터 효율 미세조정 단계</label>
  <div id="genai40-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">LoRA·LoHa·LoKr</div>
  <input id="genai40" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai40 = document.getElementById('genai40');
    var genai40Out = document.getElementById('genai40-out');
    var genai40Steps = ["LoRA·LoHa·LoKr","PEFT 도구","가중치 합성"];
    genai40.addEventListener('input', function () {
      genai40Out.textContent = genai40Steps[Number(genai40.value) - 1];
    });
  </script>
</div>
```
