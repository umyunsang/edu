...
> [!important] 세 구성요소의 역할을 섞지 않는다
> 자료에서 checkpoint는 기존 학습 모델, LoRA는 새 이미지나 스타일을 학습한 추가 파일, VAE는 흐린 이미지나 표현을 보정하는 요소다. 각 이름을 source가 부여한 역할에 맞춰 읽는다.
...
Slider는 자료가 먼저 명시한 0.1–1.0 범위만 표현한다. 값 1은 100% 적용이라는 원문 설명을 함께 보여 주지만, 가장 좋은 가중치나 제품별 권장값은 source에 없다.

> [!warning] 가중치 시작점이 source 내부에서 충돌한다
> 같은 설명은 “가중치는 0.1–1.0까지 반영할 수 있다”고 적은 뒤 “0부터 각 단계별 이미지 변화를 보여 준다”고도 말한다. 0.1과 0은 시작점이 서로 맞지 않는다. 이 노트와 slider는 명시된 범위 0.1–1.0을 사용하고, 0부터라는 문구는 원문 내부 충돌로 남긴다. 1은 100% 적용이라는 뜻이지 품질 최고점이라는 뜻이 아니다.

## Civitai에서 배포 모델을 찾는 흐름

강의 자료는 Stable Diffusion의 checkpoint·LoRA·VAE·embedding 적용 주제를 나열한 뒤 Civitai의 models 화면을 제시한다. 이어지는 BlindBox Artstyle도 Civitai 모델 페이지 사례다. 이 배치는 Civitai를 이미 학습된 모델과 스타일 파일을 찾아 적용하는 배포 경로의 사례로 보여 준다.

자료가 확인해 주는 범위는 models 목록과 BlindBox 모델 페이지까지다. 다운로드 수, 평점, 라이선스 조건, 모델 간 성능 순위는 추출 텍스트에 없다. 따라서 Civitai가 등장한다는 사실을 모델 탐색·배포 맥락으로만 쓰고, 플랫폼 품질이나 안전성을 추가 평가하지 않는다.

> [!note] Civitai 사례에서 확인 가능한 것
> Stable Diffusion에 추가할 모델을 찾는 models 화면과 BlindBox Artstyle의 버전 설명이 연결된다. 화면에 보이지 않는 세부 조건은 이 source만으로 채우지 않는다.
...
이 사례를 단순히 “V3가 더 좋다”로 요약하면 자료의 핵심 경계가 사라진다. V3의 변경점은 눈이고, 현재 상태는 실험적이며, 안정성 추천은 v1_mix다. 어느 버전이 더 우수한지 판단할 비교 점수나 시험 횟수는 추출 텍스트에 없다.
...
| Caption과 데이터 준비 | mannequin 영향 분리 | 자료는 caption과 proper dataset preparation을 함께 요구함 |
...
- **광고 디자인** — 특정 브랜드의 이미지와 스타일에 맞춘 광고 이미지를 자동 생성한다.
- **게임 개발** — 게임 캐릭터나 배경 이미지를 텍스트 설명으로 생성한다.
- **예술 창작** — 작가의 스타일에 맞춘 예술 작품을 생성한다.
- **교육 자료 제작** — 교육 콘텐츠와 관련된 이미지를 생성한다.

이 네 분야는 source가 Stable Diffusion fine-tuning의 응용 사례로 직접 열거한 것이다. 분야 이름만으로 실제 품질·비용·권리 검토가 완료됐다고 확장하지 않는다.

> [!tip] DreamBooth와 LoRA 설명의 범위
> 자료는 DreamBooth에 소수 이미지·세밀한 특징·전용 모델을, LoRA에 저랭크 부분 갱신·효율성·빠른 학습·적은 데이터 요구를 연결한다. 동일 데이터로 두 방법을 비교한 수치는 제공하지 않는다.
