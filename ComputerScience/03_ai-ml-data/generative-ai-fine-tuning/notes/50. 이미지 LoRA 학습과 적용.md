...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai50" style="font-size:14px;font-weight:600">50. 이미지 LoRA 학습과 적용 단계</label>
  <div id="genai50-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">Stable Diffusion 구성</div>
  <input id="genai50" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai50 = document.getElementById('genai50');
    var genai50Out = document.getElementById('genai50-out');
    var genai50Steps = ["Stable Diffusion 구성","의류 데이터 8단계","DreamBooth 비교"];
    genai50.addEventListener('input', function () {
      genai50Out.textContent = genai50Steps[Number(genai50.value) - 1];
    });
  </script>
</div>
```
...
<Tabs>
  <Tab label="설치·생성 항목">Windows 설치, 체크포인트 추가, txt2img, img2img가 자료의 기초 사용 항목으로 나열된다. 이 구간에는 각 절차의 본문이 없다.</Tab>
  <Tab label="설명된 구성요소">LoRA는 체크포인트에 새 대상·스타일을 더하고, VAE는 흐린 결과를 보정하는 역할로 설명된다.</Tab>
  <Tab label="링크 제목만 남은 항목">embedding 적용, 배경 삭제, 원하는 자세, 태그 키워드, Civitai가 제목·주소 수준으로만 남아 있다. 세부 기능과 절차는 이 노트에서 확장하지 않는다.</Tab>
</Tabs>

```mermaid
flowchart TB
    Base[체크포인트와 LoRA] --> Generate[이미지 생성]
    Generate --> VAE[VAE 보정]
    VAE --> Output[최종 이미지]
```
...
<div style="font-family:system-ui,sans-serif;padding:20px">
  ...
  <script>
    var stats = [
      ...
      ['의류 학습 이미지', '1–20장', '많을수록 유리', 'var(--chart-1)'],
      ['데이터 지침', '8단계', '원문 순서', 'var(--chart-5)']
    ];
