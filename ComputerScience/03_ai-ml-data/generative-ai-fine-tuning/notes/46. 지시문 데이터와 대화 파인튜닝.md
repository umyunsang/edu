...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai46" style="font-size:14px;font-weight:600">46. 지시문 데이터와 대화 파인튜닝 단계</label>
  <div id="genai46-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">Alpaca 시드</div>
  <input id="genai46" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai46 = document.getElementById('genai46');
    var genai46Out = document.getElementById('genai46-out');
    var genai46Steps = ["Alpaca 시드","Vicuna 대화","Vision LoRA"];
    genai46.addEventListener('input', function () {
      genai46Out.textContent = genai46Steps[Number(genai46.value) - 1];
    });
  </script>
</div>
```
