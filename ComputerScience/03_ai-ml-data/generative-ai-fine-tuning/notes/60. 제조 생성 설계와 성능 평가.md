...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai60" style="font-size:14px;font-weight:600">60. 제조 생성 설계와 성능 평가 단계</label>
  <div id="genai60-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">제조 DX</div>
  <input id="genai60" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai60 = document.getElementById('genai60');
    var genai60Out = document.getElementById('genai60-out');
    var genai60Steps = ["제조 DX","3D CAD·토폴로지","다중 평가"];
    genai60.addEventListener('input', function () {
      genai60Out.textContent = genai60Steps[Number(genai60.value) - 1];
    });
  </script>
</div>
```
