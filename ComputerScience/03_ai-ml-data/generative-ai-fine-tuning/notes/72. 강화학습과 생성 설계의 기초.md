...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai72" style="font-size:14px;font-weight:600">72. 강화학습과 생성 설계의 기초 단계</label>
  <div id="genai72-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">Physical AI</div>
  <input id="genai72" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai72 = document.getElementById('genai72');
    var genai72Out = document.getElementById('genai72-out');
    var genai72Steps = ["Physical AI","GPU 시뮬레이션","Eureka 보상"];
    genai72.addEventListener('input', function () {
      genai72Out.textContent = genai72Steps[Number(genai72.value) - 1];
    });
  </script>
</div>
```
