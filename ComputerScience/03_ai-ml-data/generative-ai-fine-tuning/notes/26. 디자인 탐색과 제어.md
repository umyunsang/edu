...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="genai26" style="font-size:14px;font-weight:600">26. 디자인 탐색과 제어 단계</label>
  <div id="genai26-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">비용 곡선</div>
  <input id="genai26" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 해당 PDF의 학습 순서를 확인합니다.</p>
  <script>
    var genai26 = document.getElementById('genai26');
    var genai26Out = document.getElementById('genai26-out');
    var genai26Steps = ["비용 곡선","생성 설계 탐색","Stable Diffusion 제어"];
    genai26.addEventListener('input', function () {
      genai26Out.textContent = genai26Steps[Number(genai26.value) - 1];
    });
  </script>
</div>
```
...
| HunyuanVideo·HunyuanWorld | text-to-video와 HunyuanWorld | 공개 대형 영상 모델 |
...
BDAA는 건축 설계와 열 성능 전문 분야를 교육, advocacy, professional excellence로 높이는 사명을 제시한다. 이 전문성 맥락은 자동 탐색의 결과를 사람이 평가해야 하는 이유와 연결된다.
...
| 모델 생태계 | 언어·영상 모델 계열이 병렬로 확장 | 기사식 우위 표현을 benchmark로 확대하지 않음 |
