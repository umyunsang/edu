...
<svg viewBox="0 0 720 128" width="100%" role="img" aria-label="특징 공학의 반복 과정을 보여주는 독자 제작 다섯 단계 도식"><text x="18" y="23" font-size="15" font-weight="700" fill="currentColor">특징 공학은 변환 한 번이 아니라 검증을 포함한 반복 과정이다</text><g transform="translate(28 49)" font-family="system-ui,sans-serif"><g fill="var(--card)" stroke="var(--border)"><rect width="110" height="48" rx="10"/><rect x="140" width="110" height="48" rx="10"/><rect x="280" width="110" height="48" rx="10"/><rect x="420" width="110" height="48" rx="10"/><rect x="560" width="110" height="48" rx="10"/></g><g font-size="13" font-weight="700" fill="currentColor" text-anchor="middle"><text x="55" y="29">이해</text><text x="195" y="29">정제</text><text x="335" y="29">변환</text><text x="475" y="29">선택</text><text x="615" y="29">검증</text></g><g stroke="var(--muted-foreground)" stroke-width="3"><path d="M116 24h18"/><path d="M256 24h18"/><path d="M396 24h18"/><path d="M536 24h18"/></g></g></svg>
</div>
```
...
```html preview h=185
<div style="font-family:system-ui,sans-serif;padding:14px;color:var(--foreground);background:var(--card);border:1px solid var(--border);border-radius:var(--radius)">
<svg viewBox="0 0 720 138" width="100%" role="img" aria-label="XOR 데이터에서 대각선 두 점이 같은 클래스를 이루는 독자 제작 산점도"><text x="18" y="23" font-size="15" font-weight="700" fill="currentColor">XOR에서는 같은 클래스가 대각선으로 마주 보인다</text><g transform="translate(120 48)" font-family="system-ui,sans-serif"><line x1="0" y1="80" x2="190" y2="80" stroke="var(--border)" stroke-width="2"/><line x1="0" y1="80" x2="0" y2="0" stroke="var(--border)" stroke-width="2"/><circle cx="28" cy="63" r="11" fill="var(--chart-1)"/><circle cx="162" cy="17" r="11" fill="var(--chart-1)"/><circle cx="28" cy="17" r="11" fill="var(--chart-5)"/><circle cx="162" cy="63" r="11" fill="var(--chart-5)"/><text x="210" y="27" font-size="14" font-weight="700" fill="var(--chart-1)">클래스 A</text><text x="210" y="48" font-size="14" font-weight="700" fill="var(--chart-5)">클래스 B</text><text x="330" y="39" font-size="14" fill="var(--muted-foreground)">한 직선이 아니라</text><text x="330" y="60" font-size="14" fill="var(--muted-foreground)">표현 방식을 바꿔야 한다</text></g></svg>
</div>
```
...
```html preview h=185
<div style="font-family:system-ui,sans-serif;padding:14px;color:var(--foreground);background:var(--card);border:1px solid var(--border);border-radius:var(--radius)">
<svg viewBox="0 0 720 138" width="100%" role="img" aria-label="직선 경계가 XOR의 네 점을 모두 구분하지 못하는 독자 제작 도식"><text x="18" y="23" font-size="15" font-weight="700" fill="currentColor">직선 하나로 XOR의 두 클래스를 모두 분리할 수 있을까?</text><g transform="translate(157 47)"><rect width="165" height="77" rx="11" fill="var(--card)" stroke="var(--border)"/><line x1="15" y1="63" x2="150" y2="13" stroke="var(--chart-3)" stroke-width="4"/><circle cx="31" cy="58" r="9" fill="var(--chart-1)"/><circle cx="135" cy="18" r="9" fill="var(--chart-1)"/><circle cx="31" cy="18" r="9" fill="var(--chart-5)"/><circle cx="135" cy="58" r="9" fill="var(--chart-5)"/><text x="82" y="102" text-anchor="middle" font-size="12" fill="var(--muted-foreground)">선형 경계: 항상 일부를 틀림</text><path d="M217 38h83" stroke="var(--muted-foreground)" stroke-width="3"/><path d="M300 38l-10-7v14z" fill="var(--muted-foreground)"/><rect x="316" width="165" height="77" rx="11" fill="var(--card)" stroke="var(--border)"/><path d="M339 59 C368 4 436 5 461 57" fill="none" stroke="var(--chart-4)" stroke-width="4"/><circle cx="339" cy="59" r="9" fill="var(--chart-1)"/><circle cx="460" cy="59" r="9" fill="var(--chart-1)"/><circle cx="366" cy="17" r="9" fill="var(--chart-5)"/><circle cx="434" cy="17" r="9" fill="var(--chart-5)"/><text x="398" y="102" text-anchor="middle" font-size="12" fill="var(--muted-foreground)">표현 확장: 비선형 경계 후보</text></g></svg>
</div>
```
...
```html preview h=190
<div style="font-family:system-ui,sans-serif;padding:14px;color:var(--foreground);background:var(--card);border:1px solid var(--border);border-radius:var(--radius)">
<svg viewBox="0 0 720 142" width="100%" role="img" aria-label="XOR에서 선형 모델의 오분류를 분석하고 다음 실험을 설계하는 독자 제작 피드백 도식"><text x="18" y="23" font-size="15" font-weight="700" fill="currentColor">실습 결과는 점수보다 “어떤 점을 왜 틀렸는가”를 알려준다</text><g transform="translate(48 51)" font-family="system-ui,sans-serif"><g><rect width="160" height="58" rx="11" fill="var(--card)" stroke="var(--border)"/><text x="80" y="24" text-anchor="middle" font-size="14" font-weight="700" fill="currentColor">예측과 정답 비교</text><text x="80" y="43" text-anchor="middle" font-size="11" fill="var(--muted-foreground)">오분류 위치 확인</text></g><path d="M178 29h65" stroke="var(--muted-foreground)" stroke-width="3"/><path d="M243 29l-10-7v14z" fill="var(--muted-foreground)"/><g transform="translate(258)"><rect width="160" height="58" rx="11" fill="var(--chart-5)"/><text x="80" y="24" text-anchor="middle" font-size="14" font-weight="700" fill="white">표현력 진단</text><text x="80" y="43" text-anchor="middle" font-size="11" fill="white">직선 경계의 한계</text></g><path d="M388 29h65" stroke="var(--muted-foreground)" stroke-width="3"/><path d="M453 29l-10-7v14z" fill="var(--muted-foreground)"/><g transform="translate(468)"><rect width="160" height="58" rx="11" fill="var(--chart-2)"/><text x="80" y="24" text-anchor="middle" font-size="14" font-weight="700" fill="white">다음 실험</text><text x="80" y="43" text-anchor="middle" font-size="11" fill="white">특징 확장·모델 변경</text></g></g></svg>
</div>
```
...
- **applies** — [04. Quantum Feature Space](<./04. Quantum Feature Space.md>) : 여기서 막힌 문제를 고차원 공간으로 푸는 시도
