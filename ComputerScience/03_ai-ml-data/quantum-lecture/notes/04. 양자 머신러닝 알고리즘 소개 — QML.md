...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <h3 style="margin:0 0 14px;font-size:16px">파라미터 수 비교, 강의 예시</h3>
  <div style="display:flex;align-items:flex-end;gap:24px;height:170px;border-bottom:1px solid var(--border);padding:8px 20px">
    <div style="flex:1;height:100%;background:var(--chart-4);border-radius:var(--radius) var(--radius) 0 0;position:relative"><span style="position:absolute;top:-22px;font-weight:700">199</span><div style="position:absolute;bottom:-24px;width:100%;text-align:center;font-size:12px">C-GAN</div></div>
    <div style="flex:1;height:5%;background:var(--chart-2);border-radius:var(--radius) var(--radius) 0 0;position:relative"><span style="position:absolute;top:-22px;font-weight:700">10</span><div style="position:absolute;bottom:-24px;width:100%;text-align:center;font-size:12px">QuGAN</div></div>
  </div>
  <p style="font-size:12px;color:var(--muted-foreground);margin:38px 0 0">원본 p.6·p.26의 파라미터 예시를 비율로 그린 도식이다. 슬라이드는 유사 성능과 94.98% 감소를 함께 주장한다.</p>
</div>
```
...
| 관찰된 장점 | 함께 기록해야 할 조건 |
| --- | --- |
| C-GAN 199개 대비 QuGAN 10개 파라미터 예시 | 강의의 특정 작업 비교 |
| 양자 상태를 직접 생성할 가능성 | 큐비트 수와 시뮬레이터 비용에 의존 |
| 충실도 기반 판별과 안정적 수렴 주장 | 원문이 제시한 실험 맥락 안에서만 해석 |
| 고차원 분포 탐색 잠재력 | 노이즈·모드 붕괴·확장성 위험 |

> [!WARNING]
> 원본 p.27 표의 파라미터 감소율은 “9498%”로 인쇄되어 p.6·p.26의 “94.98%”와 표기가 불일치한다. 원문 오류를 수정하지 않고 불일치 자체를 기록하며, 여기서는 후자의 표기를 강의 내 반복 수치로만 구분한다.
