...
단일 큐비트 상태 $|\psi\rangle$에 게이트 $U$를 적용하면 새 상태는 $U|\psi\rangle$가 된다. 유니터리 조건은 전체 확률을 보존하고, 적용 전 상태로 되돌릴 역연산이 있음을 뜻한다.
...
X 게이트는 계산 기저에서 $|0\rangle$과 $|1\rangle$을 바꾸므로 비트 반전처럼 보인다. 그러나 중첩 상태까지 포함하면 X는 블로흐 구면의 한 축을 기준으로 한 회전으로 이해하는 편이 정확하다.
...
$R_y(\theta)$는 회전각 $\theta$로 중첩의 비율을 제어한다. $|0\rangle$에서 시작한 경우 두 결과의 확률은 아래처럼 변한다.
...
```html preview
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <h3 style="margin:0 0 14px;font-size:15px;font-weight:600">R_y(90°)에서의 측정 확률</h3>
...
> [!caution] 이 차트의 범위
> 이 비교는 $|0\rangle$에서 시작하는 $R_y(90^\circ)$만 나타낸다. 일반 상태의 복소 위상이나 측정 뒤 상태 갱신까지 뜻하지는 않는다.
...
<div style="font-family:system-ui,sans-serif;padding:20px;display:flex;align-items:center;gap:20px;color:var(--foreground)">
  <svg width="120" height="120" viewBox="0 0 120 120" role="img" aria-label="50 percent probability for each of two measurement outcomes">
  ...
  <div>
    <div style="font-weight:600;font-size:15px">균등한 측정 분포</div>
    <div style="font-size:13px;color:var(--muted-foreground);margin-top:2px">H 뒤의 계산 기저 측정에서 각 결과가 50%</div>
  </div>
</div>
```
...
> [!question]- 스스로 점검
> **Q.** $R_y(0)|0\rangle$와 $R_y(180^\circ)|0\rangle$의 측정 분포는 어떻게 다른가?
>
> **A.** 전자는 $|0\rangle$에, 후자는 $|1\rangle$에 확률 100%가 놓인다.
