...
LUCJ는 Hartree–Fock 기준 상태 (|Phi_0angle)에서 시작해 (e^K) orbital rotation, (e^{iJ}) 상호작용, 측정으로 이어지는 회로 구조로 표시된다. 슬라이드는 이를 물리적 타당성과 하드웨어 효율성을 함께 갖춘 특화 양자 회로 구조로 설명한다.
...
| 부분공간 구축 | Born 확률 기반 중요한 행렬식 | (S = operatorname{span}{lvert D_iangle}) 구성 |
