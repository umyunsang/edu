...
description: "기존 관계형 데이터베이스로 처리 불가능한 대용량 데이터의 특성(3V: Volume, Velocity, Variety), 자연어 및 웹 데이터의 불균형 분포를 설명하는 지프의 법칙(Zipf's Law)과 멱법칙(Power Law: P(x) \propto x^{-\alpha}), 문서 내 단어 중요도를 평가하는 TF-IDF 가중치 수식, 그리고 빅데이터 분석 생애주기를 인터랙티브 TF-IDF 및 지프의 법칙 시뮬레이터로 학습한다."
type: lecture
tags:
  - big-data-analysis
  - 3v-model
