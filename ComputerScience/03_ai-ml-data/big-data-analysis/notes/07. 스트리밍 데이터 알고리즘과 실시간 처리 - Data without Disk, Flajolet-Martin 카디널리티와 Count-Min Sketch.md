...
description: "디스크 저장 없이 단 한 번의 순회(Single Pass)와 극도로 제한된 $O(\text{polylog } N)$ 메모리로 방대한 데이터 스트림을 분석하는 스트리밍 알고리즘(Streaming Algorithms), 서로 다른 원소 개수를 추정하는 Flajolet-Martin(FM / HyperLogLog) 카디널리티 알고리즘, 빈도수를 추정하는 Count-Min Sketch(해시 함수 $d$개와 2차원 배열), 그리고 슬라이딩 윈도우(Sliding Window) 기반 DGIM 알고리즘을 인터랙티브 Flajolet-Martin 카디널리티 계산기로 학습한다."
type: lecture
tags:
  - big-data-analysis
