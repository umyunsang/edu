...
## 5. 문자열과 그래프로 — 같은 기술의 세 응용

### 5.1 최장 공통 부분순서 (LCS)

원본이 드는 쓰임새가 이 문제의 가치를 말해 준다(34쪽) — **데이터의 유사도 평가**, 구체적으로는 *유전자 염기 서열, 두 소스 파일의 차이 등*. `git diff` 가 내부에서 하는 일이 바로 이것이다.

순환식은 마지막 문자를 기준으로 나눈다(35쪽).
...
$$
L(i, j) = \begin{cases}
L(i-1, j-1) + 1 & x_i = y_j \quad \text{(둘 다 쓴다)} \\
\max\bigl(L(i-1, j),\; L(i, j-1)\bigr) & x_i \neq y_j \quad \text{(한쪽을 포기)}
\end{cases}
$$

마지막 문자가 같으면 둘 다 쓰고 대각선으로 내려가고, 다르면 한쪽을 버리는 두 선택 중 나은 쪽을 택한다. 테이블 크기는 $(m+1) \times (n+1)$ 이고 시간·공간 모두 $O(mn)$ 다(37쪽).

표를 다 채우면 **길이**만 나오므로, 실제 문자열을 얻으려면 되돌아가며 추적해야 한다(38쪽). 이 “표를 채우고 → 역추적하여 해 복원” 구조는 배낭·편집거리에도 그대로 적용된다.

### 5.2 플로이드-워셔 — 경유지를 하나씩 허용하기

최단 경로 문제는 세 종류로 나뉩니다(42쪽) — 두 정점 사이, 한 정점에서 모든 정점, **모든 정점 간**. 마지막이 7.6절의 플로이드 알고리즘이다.

아이디어는 경유지를 한 개씩 풀어주는 것이다. $D_k[i][j]$ 를 *1~$k$번째 정점까지**만*을 이용한 $i \to j$ 최단 경로*로 두면:

$$
D_0 = W \quad (\text{아무런 정점도 거치지 않는 경로 — 인접 행렬 그대로})
$$

$$
D_k[i][j] = \min\bigl(\underbrace{D_{k-1}[i][j]}_{k\text{를 안 거침}},\; \underbrace{D_{k-1}[i][k] + D_{k-1}[k][j]}_{k\text{를 거침}}\bigr)
$$

전략은 $D_0 \to D_1 \to D_2 \to \cdots \to D_n$ 이고, 최적해는 $D_n$ 이다(44쪽). 3중 루프로 구현되어 $O(n^3)$(46쪽).

> [!warning] 최장 경로에는 쓸 수 없다
> 원본 43쪽의 대조가 날카롭다 — 최단 경로 문제는 **최적 부분 구조 특성을 만족**하지만, 최장 경로 문제는 **만족하지 않는다**.
> 이유는 직관적이다. 최단 경로의 부분 경로는 반드시 최단 경로다(아니면 더 짧게 바꿔치면 되므로). 반면 최장 경로의 부분 경로는 최장이 아닐 수 있다 — 부분을 늘리려다 정점을 재방문하게 되어 경로가 성립하지 않을 수 있기 때문이다.
> **두 조건 중 하나만 깨져도 DP는 쓸 수 없다**는 것을 보여주는 가장 깔끔한 예다.

### 5.3 편집 거리

두 문자열을 같게 만드는 데 필요한 최소 편집 연산 횟수다(48~49쪽). 기반 상황은 *한쪽이 빈 문자열이면 다른 쪽 길이만큼 연산이 필요*.

$$
\text{edit}(m, 0) = m, \qquad \text{edit}(0, n) = n
$$

일반 상황은 마지막 문자가 같은지로 갈라진다.

| 경우 | 연산 | 식 |
| --- | --- | --- |
| $S[m] = T[n]$ | 아무것도 안 함 | $\text{edit}(m-1, n-1)$ |
| $S[m] \neq T[n]$ | **대체** — $S[m]$ 을 $T[n]$ 으로 | $\text{edit}(m-1, n-1) + 1$ |
| | **삭제** — $S[m]$ 을 지움 | $\text{edit}(m-1, n) + 1$ |
| | **삽입** — $T[n]$ 을 넣음 | $\text{edit}(m, n-1) + 1$ |

세 연산 중 최솟값을 취한다. LCS와 구조가 거의 같지만 **최대화가 아니라 최소화**이고, 대각선 이동에도 비용이 붙을 수 있다는 점이 다르다.

---

## 정리 — 이 장의 모든 문제는 같은 절차를 밟는다

```mermaid
flowchart TD
    S1["① 상태를 정의한다<br/>변수 개수 = 표의 차원"]
    S1 --> S2["② 기반 상황을 적는다<br/>답이 자명한 가장 작은 문제"]
    S2 --> S3["③ 마지막 선택으로 경우를 나눈다<br/>쓰느냐 / 마느냐 · 같으냐 / 다르냐"]
    S3 --> S4["④ 순환 관계식을 완성한다"]
    S4 --> S5["⑤ 표를 채운다<br/>메모이제이션 또는 테이블화"]
    S5 --> S6["⑥ 필요하면 역추적해 해를 복원한다"]
```
...
> [!tip] 세 문장 요약
>
> 1. **DP는 겹치는 부분 문제와 최적 부분 구조가 모두 있을 때만 쓴다.** 하나라도 없으면 다른 기법을 찾아야 한다 — 최장 경로가 그 반례다.
> 2. **순환식을 세우는 일이 전부다.** 기반 상황을 정하고, 마지막 선택으로 경우를 나누면 표 채우기는 기계적이다.
> 3. **조건이 붙으면 상태에 차원을 더한다.** 격자 경로 3이 보여준 확장 기술이며, 대가는 공간이다.

다음 장은 정반대 질문을 던진다 — **모든 경우를 비교하지 않고, 매 순간 좋아 보이는 것만 골라도 되는 문제는 없을까?** 이것이 [탐욕적 기법](./08.%20%ED%83%90%EC%9A%95%EC%A0%81%20%EA%B8%B0%EB%B2%95.md)이고, 위 거스름돈 반례가 그 위험을 이미 보여줬다.

---

### 근거 자료

강의 원본 `07장-동적계획법-수정판.pdf` (53쪽)에 근거한다 — 원본 28쪽이 안내하듯이 중간에 별도의 `07-동적계획법-보충-수정판.pdf` (63쪽)으로 강의가 이어진다. 인용 슬라이드는 9쪽(DP 두 조건), 17쪽(격자 경로 1), **22쪽(격자 경로 3 확장 필기)**, 30쪽(배낭 순환관계식)이다. 격자 경로 인터렉티브는 17쪽의 표(1·2·3… / 1·3·6·10·15·21·28·36)를 재현하며, 거스름돈 계산기의 두 프리셋은 25쪽(12원, [1,6,10])과 26쪽(25원, [1,7,15])의 표와 값이 일치함을 확인했다.
