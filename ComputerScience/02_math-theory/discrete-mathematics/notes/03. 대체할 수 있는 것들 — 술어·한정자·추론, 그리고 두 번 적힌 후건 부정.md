...
>
> 사람은 $F \land A$를 보면 $A$를 곧바로 읽어 내지만, 규칙 목록에는 그런 허가증이 없다. 그래서 교환법칙이라는 허가증을 한 장 더 써서 모양을 맞춘 뒤에야 꺼낼 수 있다. **증명이 사람의 직관이 아니라 허가증 목록 위에서 돌아간다**는 사실이 이 한 단계에 압축되어 있다. 02번 노트의 연산자 우선순위표가 괄호를 기계에게 넘긴 것과 같은 방향이다.
