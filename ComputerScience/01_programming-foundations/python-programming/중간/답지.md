...
1. 2개의 리스트를 받아서 공통 항목이 하나 이상 있으면 True를 반환하는 함수를 작성하세요. (반복문을 활용하세요.)
...
1. 로또 번호를 생성하는 코드를 작성하세요. 로또 번호는 1부터 45까지의 숫자 6개로 이루어집니다. 또한 숫자가 중복되면 안됩니다. 1에서 45까지의 난수는 random.randint(1, 45)로 만들면 됩니다.
...
1. 다음의 규칙을 갖는 수식을 계산하는 코드를 작성하세요.$$3 + \frac{4}{2 \times 3 \times 4} - \frac{4}{4 \times 5 \times 6} + \cdots$$
...
1. 1부터 99까지 2자리의 정수로 이루어진 복권 이 있다. 2자리가 전부 당첨 번호와 일치하면 ‘1등상’이고, 2자리 중 하나만 일치하면 ‘2등상’이다. 일치하는 숫자가 없으면 ‘미당첨’이다. 복권 번호 는 사용자에게 입력받고, 당첨 번호는 난수로 생 성한 후 당첨 결과를 출력하는 코드를 작성하세요
