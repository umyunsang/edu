---
title: 06. 스레드 동기화
description: 공유 데이터에 대한 동시 접근을 순서화해 훼손을 막는 동기화 기법
type: lecture
tags:
- operating-systems
- systems
course: operating-systems
semester: 2-2
source: 6. 스레드 동기화.pdf
status: draft
---

> [!abstract]
> 스레드 동기화는 여러 스레드가 공유 데이터에 접근해도 결과가 훼손되지 않도록 접근 순서를 제어하는 방법이다.

## 핵심 구조

<Tabs>
<Tab title="임계 구역">공유 데이터를 갱신하는 구간</Tab>
<Tab title="상호 배제">한 번에 하나의 스레드만 진입</Tab>
<Tab title="도구">뮤텍스·스핀락·세마포와 원자 명령</Tab>
</Tabs>

```mermaid
flowchart LR
    A[요청] --> B[운영체제 판단]
    B --> C[자원 또는 상태 관리]
    C --> D[실행 결과]
```

> [!important]
> 공유 데이터를 갱신하는 구간와 한 번에 하나의 스레드만 진입은 별개가 아니라, 운영체제가 실행을 관리할 때 함께 고려하는 관점이다.

```html preview h=145px
<div style="font-family:system-ui,sans-serif;padding:16px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--foreground)"><strong>06. 스레드 동기화</strong><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:13px"><span style="padding:10px;border:1px solid var(--border);border-radius:8px">임계 구역</span><span style="padding:10px;border:1px solid var(--border);border-radius:8px">상호 배제</span><span style="padding:10px;border:1px solid var(--border);border-radius:8px">도구</span></div></div>
```

<details>
<summary>읽기와 쓰기가 겹치면?</summary>

실행 순서에 따라 관찰값이 달라질 수 있으므로 보호 범위를 설계한다.
</details>

## 판단과 검증

==정책의 이름만 외우기보다, 어떤 요청·자원·상태 변화에 적용되는지 추적하는 것이 핵심이다.==

```html preview h=145px
<div style="font-family:system-ui,sans-serif;padding:16px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--foreground)"><div style="display:flex;gap:10px;align-items:center"><span style="padding:10px;background:var(--chart-1);color:var(--background);border-radius:8px">관찰</span><b>→</b><span style="padding:10px;background:var(--chart-2);color:var(--background);border-radius:8px">선택</span><b>→</b><span style="padding:10px;background:var(--chart-3);color:var(--background);border-radius:8px">검증</span></div><p style="margin:12px 0 0">상태와 정책의 결과를 함께 기록한다.</p></div>
```

<details>
<summary>어떤 비교가 필요한가?</summary>

같은 입력 조건에서 정책을 바꿔 보고, 성능뿐 아니라 공정성·보호·자원 사용량의 차이를 관찰한다.
</details>

> [!warning]
> 한 가지 지표만 좋다고 전체 시스템의 선택이 자동으로 정당화되지는 않는다. 충돌하는 목표와 예외 상황을 함께 점검한다.

다음 관찰을 학습 흐름에 연결한다.

> [!tip]
> 학습 팁: 요청이 들어온 순간부터 운영체제가 바꾸는 상태를 화살표로 직접 그려 본다.

### 스스로 점검

- 임계 구역의 역할을 한 문장으로 설명할 수 있는가?
- 상호 배제와 도구가 어떤 상황에서 연결되는가?
- 정책을 비교할 때 무엇을 측정해야 하는가?
