---
title: 04. 스레드와 멀티태스킹
description: 프로세스 안에서 실행 흐름을 분리해 생성과 전환 비용을 낮추는 스레드
type: lecture
tags:
- operating-systems
- systems
...
> [!abstract]
> 스레드는 현대 운영체제가 스케줄하는 실행 단위이며, 한 프로세스 안에서 여러 작업 흐름을 만들 수 있다.

## 핵심 구조

<Tabs>
<Tab title="비용">프로세스 생성과 컨텍스트 전환의 부담</Tab>
<Tab title="공유">같은 프로세스의 주소 공간을 함께 사용</Tab>
<Tab title="구현">커널·사용자 수준과 멀티스레딩 모델</Tab>
</Tabs>
...
> [!important]
> 프로세스 생성과 컨텍스트 전환의 부담와 같은 프로세스의 주소 공간을 함께 사용은 별개가 아니라, 운영체제가 실행을 관리할 때 함께 고려하는 관점이다.

```html preview h=145px
<div style="font-family:system-ui,sans-serif;padding:16px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--foreground)"><strong>04. 스레드와 멀티태스킹</strong><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:13px"><span style="padding:10px;border:1px solid var(--border);border-radius:8px">비용</span><span style="padding:10px;border:1px solid var(--border);border-radius:8px">공유</span><span style="padding:10px;border:1px solid var(--border);border-radius:8px">구현</span></div></div>
```

<details>
<summary>공유가 언제 위험해지는가?</summary>

같은 데이터에 쓰기가 겹치면 동기화가 필요하다.
</details>
...
- 비용의 역할을 한 문장으로 설명할 수 있는가?
- 공유와 구현가 어떤 상황에서 연결되는가?
