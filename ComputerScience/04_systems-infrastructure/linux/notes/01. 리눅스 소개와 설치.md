...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  ...
  <script>
    var data = ["소개",1],["설치",2],["둘러보기",3]("소개",1],["설치",2],["둘러보기",3.md);
...
```html preview
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow601300" style="font-size:14px;font-weight:600">리눅스 소개와 설치 단계</label>
  <div id="noteFlow601300-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">리눅스 소개와 설치 핵심 개념</div>
  <input id="noteFlow601300" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow601300 = document.getElementById('noteFlow601300');
    var noteFlow601300Out = document.getElementById('noteFlow601300-out');
    var noteFlow601300Steps = ["리눅스 소개와 설치 핵심 개념","리눅스 소개와 설치 처리 절차","리눅스 소개와 설치 검증·응용"];
    noteFlow601300.addEventListener('input', function () { noteFlow601300Out.textContent = noteFlow601300Steps[Number(noteFlow601300.value) - 1]; });
  </script>
</div>
```
