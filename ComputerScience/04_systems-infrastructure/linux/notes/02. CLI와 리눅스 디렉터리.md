---
title: 02. CLI와 리눅스 디렉터리
description: 명령줄 인터페이스로 명령을 입력하고 파일 계층을 탐색하는 기초
type: lecture
tags:
- linux
- systems
course: linux
semester: 2-2
source: 리눅스-2주차.pdf
status: draft
---

> [!abstract]
> CLI는 터미널에서 문자열 명령을 입력하고 결과를 받아 컴퓨터와 상호작용하는 방식이다.

## 핵심 구조

<Tabs>
<Tab title="입력">명령과 인자</Tab>
<Tab title="출력">텍스트 결과와 오류</Tab>
<Tab title="구조">디렉터리 계층과 현재 위치</Tab>
</Tabs>

```mermaid
flowchart LR
 A[사용자 요청] --> B[리눅스 인터페이스]
 B --> C[운영체제 기능]
 C --> D[결과]
```

> [!important]
> 명령과 인자·텍스트 결과와 오류·디렉터리 계층과 현재 위치은 실제 작업 흐름에서 연결되어야 한다.

```html preview h=135px
<div style="font-family:system-ui,sans-serif;padding:14px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--foreground)"><strong>02. CLI와 리눅스 디렉터리</strong><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px"><span style="padding:8px;border:1px solid var(--border);border-radius:7px">입력</span><span style="padding:8px;border:1px solid var(--border);border-radius:7px">출력</span><span style="padding:8px;border:1px solid var(--border);border-radius:7px">구조</span></div></div>
```

<details>
<summary>GUI가 있어도 CLI를 배우는 이유는?</summary>

명령 조합과 자동화, 원격 환경 작업에 CLI가 유용하다.
</details>

## 점검 흐름

==명령이나 설정을 외우기보다 입력·권한·실행 결과를 연결해 확인한다.==

```html preview h=125px
<div style="font-family:system-ui,sans-serif;padding:14px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--foreground)"><div style="display:flex;gap:9px;align-items:center"><span style="padding:8px;background:var(--chart-1);color:var(--background);border-radius:7px">입력</span><b>→</b><span style="padding:8px;background:var(--chart-2);color:var(--background);border-radius:7px">권한</span><b>→</b><span style="padding:8px;background:var(--chart-3);color:var(--background);border-radius:7px">결과</span></div></div>
```

<details><summary>실습 전 확인</summary>

작업 대상과 현재 사용자, 필요한 권한을 먼저 확인하고 결과를 기록한다.
</details>

> [!warning]
> 시스템 작업은 잘못된 대상이나 권한으로 실행하면 예상보다 큰 영향을 줄 수 있다.

다음 확인을 실제 작업 순서에 넣는다.

> [!tip]
> 작은 예시에서 먼저 실행 결과를 관찰하고, 같은 절차를 확장한다.

### 스스로 점검

- 입력의 역할을 설명할 수 있는가?
- 출력와 구조가 어떤 순서로 연결되는가?
- 실행 전 무엇을 확인해야 하는가?
