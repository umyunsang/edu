...
- 인터럽트 사이클
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__명령어 사이클.png -->
...
- 명령어 사이클 흐름
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__명령어 사이클 2.png -->
...
- 다음에 실행할 명령어를 주기억 장치에서 읽어 오는 과정
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__1. 명령어 인출 사이클 instruction fetch.png -->
- 명령어 인출 사이클이 진행되는 동안 프로세서의 레지스터 변화 과정

 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__1. 명령어 인출 사이클 instruction fetch 2.png -->
...
- 해독된 명령어에 대한 후속 마이크로 연산 발생
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__2. 명령어 해독 사이클 instruction decode.png -->
...
- 예 : 데이터를 읽어서 레지스터 R1에 저장하는 명령어 실행 사이클
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__3. 명령어 실행 사이클 instruction execute.png -->
- 명령어 인출 사이클이 진행되는 동안 프로세서의 레지스터 변화 과정

 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__3. 명령어 실행 사이클 instruction execute 2.png -->
- 명령어 실행 사이클은 명령어 세트의 개수만큼 아주 다양하게 존재
- 명령 인출과정은 동일
- 레지스터 R1의 데이터와 메모리 X번지의 데이터를 ALU에서 더해 다시 R1에 저장

 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__3. 명령어 실행 사이클 instruction execute 3.png -->

##### LOAD(1), ADD(5), STORE(2) 3개의 명령이 실행되는 과정

- CPU내의 PC, R1, IR 등의 레지스터 내용 변화 주목

 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__LOAD(1), ADD(5), STORE(2) 3개의 명령이 실행되.png -->
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__LOAD(1), ADD(5), STORE(2) 3개의 명령이 실행되 2.png -->

##### 예: ==ISZ(Increment and Skip if Zero) => if 문문==

- ==X 값을 1 증가시키고 그 결과가 0이면 바로 다음 명령을 건너뜀==

 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__예 ISZ(Increment and Skip if Zero.png -->

##### 예: ==BSA(Branch-and-Save-Address) => 함수 호출==

<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__예 BSA(Branch-and-Save-Address) 함수.png -->

##### 예: ==RET (return)==

<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__예 RET (return.png -->
<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__예 RET (return) 2.png -->
...
- 복귀할 때는 스택 값을 꺼내어 PC로 가져온다.
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__중첩 서브루틴이나 다중 서브루틴인 경우.png -->

##### ==다중 서브루틴 예에서 레지스터 변화 (시험 예상)==

<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__다중 서브루틴 예에서 레지스터 변화 (시험 예상) 2.png -->
<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__다중 서브루틴 예에서 레지스터 변화 (시험 예상.png -->
...
- 인터럽트 사이클은 통상적으로 다음과 같다.
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__4. 인터럽트 사이클 interrupt.png -->
...
- 인터럽트 사이클은 항상 명령어 실행이 끝난 후 인터럽트가 있으면 실행하고, 그렇지 않으면 다음 명령어 인출 사이클로 진행
 ...
 <!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__5. 명령어 사이클.png -->

## PNG 시각자료

<!-- 원본 이미지 없음: computer-architecture__3. 명령어 사이클__oe! mee Subt o e e.png -->
