...
<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__TTL과 CMOS 논리 레벨 정의 영역.png -->

#### NOT 게이트
- 한 개의 입력과 한 개의 출력을 갖는 게이트로 논리 부정을 나타낸다.
- NOT 게이트를 인버터(inverter)라고도 한다.
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__NOT 게이트.png -->

#### 버퍼 게이트
- 버퍼(buffer)는 입력 신호를 그대로 출력하여 단순 전송을 표현하는 게이트다.
- 입력이 0이면 0을 출력하고, 입력이 1이면 1을 출력한다.
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__버퍼 게이트.png -->
##### 3상태 버퍼 (tri-state buffer)
- 출력이 3개 레벨(Hight, Low, 하이임피던스)중의 하나를 갖는 논리소자
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__3상태 버퍼 (tri-state buffer.png -->

#### AND 게이트
- 입력이 모두 1인 경우에만 출력이 1이고, 입력 중 0이 하나라도 있으면 출력이 0이 되는 논리 곱이다.
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__AND 게이트.png -->

#### OR 게이트
- 입력이 모두 0인 경우에만 출력이 0이고, 입력 중 1이 하나라도 있으면 출력은 1이 되는 논리 합이다.
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__OR 게이트.png -->
...
- 2입력 XOR 게이트는 두 입력 중 하나가 1이면 출력이 1이고, 두 입력 모두 0이거나 1인 경우에 출력이 0이 되는 것이다.
	<!-- 원본 이미지 없음: computer-architecture__1. 논리 게이트__XOR 게이트 (eXclusive OR gate.png -->
