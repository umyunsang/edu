...
<div style="font-family:system-ui,sans-serif;padding:20px;color:var(--foreground)">
  <label for="noteFlow378422" style="font-size:14px;font-weight:600">신호 처리 단계</label>
  <div id="noteFlow378422-out" style="font-size:26px;font-weight:700;color:var(--chart-1);margin:6px 0">신호 처리 핵심 개념</div>
  <input id="noteFlow378422" type="range" min="1" max="3" step="1" value="1" style="width:100%;accent-color:var(--primary)" />
  <p style="font-size:13px;color:var(--muted-foreground)">슬라이더를 움직이면 원본 PDF에서 정리한 이 노트의 흐름을 확인합니다.</p>
  <script>
    var noteFlow378422 = document.getElementById('noteFlow378422');
    var noteFlow378422Out = document.getElementById('noteFlow378422-out');
    var noteFlow378422Steps = ["신호 처리 핵심 개념","신호 처리 처리 절차","신호 처리 검증·응용"];
    noteFlow378422.addEventListener('input', function () { noteFlow378422Out.textContent = noteFlow378422Steps[Number(noteFlow378422.value) - 1]; });
  </script>
</div>
```
