...
Cost depends on transcript length and current Codex API pricing.
...
python youtube_transcript_summarizer.py "VIDEO_URL" --model Codex-sonnet-4-5-20250929 --max-transcript-chars 30000
```
...
| `--api-key` | Codex API key | `ANTHROPIC_API_KEY` env |
| `--model` | Codex model name | `Codex-sonnet-4-5-20250929` |
| `--max-transcript-chars` | Max chars sent to Codex | `15000` |
